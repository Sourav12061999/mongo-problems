...
- Write a MongoDB query to find restaurants in the `restaurants` collection that have a `cuisine` field equal to `Italian` and return only the `name` and `address` fields.
