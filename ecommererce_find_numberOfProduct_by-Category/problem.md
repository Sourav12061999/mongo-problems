...
- Write Mongo query to retrieve the number of Products with a particular category("Electronics") as key "totalProduct" ?
- <span style="color:red">Hint:</span> use MongoDB aggregation pipeline with `$match`, `$lookup`, `$project`, `$unwind`, `$count`
