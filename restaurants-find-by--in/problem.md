...
- Write a MongoDB query to find the `restaurant_id`, `name`, `borough` and `cuisine` for those restaurants which are not belonging to the borough `Staten Island`, `Queens`, `Bronx` and `Brooklyn`.
- Hint - use `$nin`
