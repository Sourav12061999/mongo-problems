...
- Problem - write query to Group movies by `director` and calculate the `average rating` (key = avg_rating) of each director's films, sorted by `highest average rating` first ?
